...
function BackgroundRow() {
      ...
      // Fetch editable blocks per session, feed the assistant slot + user injector.
      function MessageEditInjector() {
        useLocaleTick();
        react.useEffect(() => {
          if (dshmCtx === null) return;
          let current = true;
          ...
          const refresh = () => {
            ...
            if (sessionId === undefined || typeof fetch !== "function") return;
          ...
          // An uninjected row appeared (new message, or we just switched into
          // this session): re-pull once, debounced and rate-capped; back off
          // while refreshes change nothing (e.g. reply still streaming).
          const onMiss = () => {
          ...
          const sync = () => {
            if (!current || sessionsSvc === undefined) return;
          ...
          const start = (svc) => {
            ...
            pollTimer = setInterval(() => {
              if (!current || typeof document === "undefined" || typeof document.querySelectorAll !== "function") return;
              const rows = document.querySelectorAll('.p-xYUq_actions, [class$="_actions"]');
              for (const row of rows) {
                if (row.__dshmEditInjected === true) continue;
                const flowItem = typeof row.closest === "function" ? row.closest('[data-chat-flow-kind]') : null;

window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // Bound by apply() so components can reach services outside render.
    let dshmCtx = null;
    ...
    .dshm-search-type-wrap {
      box-sizing: border-box;
      flex: 1 1 140px;
      min-width: 0;
      position: relative;
    }
    .dshm-search-type-btn {
      box-sizing: border-box;
      width: 100%;
      display: flex;
      align-items: center;
    ...
    .dshm-search-type-btn:focus-visible { outline: 2px solid var(--dsw-alias-brand-primary, #2563eb); }
    .dshm-search-type-btn-label { flex: 1; min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; text-align: left; }
    .dshm-search-type-chevron { flex: none; display: inline-flex; color: var(--dsw-alias-label-tertiary, #81858c); transition: transform 0.16s; }
    .dshm-search-type-btn.dshm-open .dshm-search-type-chevron { transform: rotate(180deg); }
    .dshm-search-type-menu {
      position: absolute;
      top: calc(100% + 4px);
      left: 0;
      right: 0;
    ...
    .dshm-search-type-option:hover { background: var(--dsw-alias-interactive-bg-hover, rgb(0 0 0 / 6%)); }
    .dshm-search-type-option-name { flex: 1; min-width: 0; }
    .dshm-search-type-option-check { flex: none; color: var(--dsw-alias-brand-primary, #2563eb); }
    ...
    .dshm-plugin-card-open { background: var(--dsw-alias-bg-layer-2, #f6f7f8); }
    ...
    .dshm-plugin-card-chevron-open { transform: rotate(180deg); }
    ...
    function apply(ctx) {
        ...
        try {
          locale.register(NS, {
            zh: {
              ...
              "searchModelTypeYou": "You.com 搜索",
              "searchModelTypeSerper": "Serper 搜索",
              "searchModelTypeSerpapi": "SerpApi 搜索",
              "searchModelTypeKagi": "Kagi 搜索",
              "searchModelTypeSearxng": "SearXNG 搜索",
              "searchModelTypeBocha": "Bocha 博查搜索",
            ...
            en: {
              ...
              "searchModelTypeYou": "You.com search",
              "searchModelTypeSerper": "Serper search",
              "searchModelTypeSerpapi": "SerpApi search",
              "searchModelTypeKagi": "Kagi search",
              "searchModelTypeSearxng": "SearXNG search",
              "searchModelTypeBocha": "Bocha search",
      ...
      function useLocaleTick() {
        const localeTickSnapshot = locale !== undefined ? () => locale.getSnapshot().active : () => "en";
...
function BackgroundRow() {
      ...
      function fetchBalances(force) {
        ...
        const now = Date.now();
        if (!force && now - lastFetchAt < 10000) return;
        lastFetchAt = now;
        fetchInFlight = true;
        fetchFailed = false;
        // Show loading indicators immediately
        injectBalances("loading");
      ...
      // Fall back to checking the element's visible text for common names.
      function providerIdOf(titleEl) {
        ...
        // Fallback: check text content for provider display names
        const text = (titleEl.textContent || "").toLowerCase();
        if (text.includes("deepseek")) return "deepseek";
        if (text.includes("openrouter")) return "openrouter";
        if (text.includes("openai") || text.includes("gpt") || text.includes("chatgpt")) return "openai";
        return null;
      }
      ...
      function fmtBalance(b) {
        ...
        let text = cur + n.toFixed(2);
        // Append total if available (OpenRouter /api/v1/credits format)
        if (b.total !== undefined && isFinite(Number(b.total))) {
          const total = Number(b.total);
          const tcur = b.currency === "CNY" ? "\u00a5" : "$";
          text += " / " + tcur + total.toFixed(2);
        }
        return text;
      }

      function injectBalances(state) {
        if (typeof document === "undefined") return;
        document.querySelectorAll("._7KE1Ra_groupTitle").forEach((el) => {
          ...
          // Build balance display element
          const wrapper = document.createElement("span");
          wrapper.setAttribute("data-dshm-balance", "");
          wrapper.style.cssText = "margin-left:8px;font-weight:500;white-space:nowrap;vertical-align:middle;display:inline-flex;align-items:center;gap:3px;";

          // State: loading, error, or data
          if (state === "loading" || (!state && fetchInFlight)) {
            wrapper.style.color = "var(--dsw-alias-label-tertiary, #8b8f93)";
            wrapper.textContent = "\u2026";
          } else if (state === "error" || fetchFailed) {
            wrapper.style.color = "var(--dsw-alias-label-tertiary, #8b8f93)";
            wrapper.textContent = "\u2014";
          } else {
            const b = balances[pid] || null;
            const text = b ? fmtBalance(b) : null;
            if (!text) {
              if (old) old.remove();
              return;
            }
            wrapper.style.color = "var(--dsw-alias-label-secondary, #61666b)";
            wrapper.textContent = text;

            // Add a small refresh button (↻) that fades in on hover
            const refreshBtn = document.createElement("span");
            refreshBtn.textContent = "\u21bb";
            refreshBtn.style.cssText = "font-size:11px;cursor:pointer;opacity:0.3;transition:opacity 0.15s;margin-left:2px;line-height:1;";
            refreshBtn.title = "Refresh balance";
            refreshBtn.addEventListener("click", function (ev) {
              ev.stopPropagation();
              retryBalances();
            });
            refreshBtn.addEventListener("mouseenter", function () { this.style.opacity = "1"; });
            refreshBtn.addEventListener("mouseleave", function () { this.style.opacity = "0.3"; });
      ...
      function usePickerOpen() {
        return react.useSyncExternalStore(subscribePicker, () => dshmPickerOpen, () => dshmPickerOpen);
      }
      ...
      // AI messages: official React slot (no DOM injection, no observer loop).
      // User messages: debounced DOM injection (they have no slot).
      ...
      function postMessageEdit(op) {
        if (typeof fetch !== "function") return Promise.resolve({ sessionId: null });
        return fetch("/api/dshm/message-edit", {
          ...
          if (r.ok && body && body.ok === true && typeof body.sessionId === "string") return { sessionId: body.sessionId };
          return { sessionId: null, error: (body && typeof body.error === "string" && body.error.length > 0) ? body.error : "操作失败，请重试" };
        }).catch(() => ({ sessionId: null, error: "操作失败，请重试" }));
      }
      function openSession(sessionId) {
        if (typeof sessionId !== "string" || sessionId.length === 0) return;
        const sessions = ctx.get("sessions");
        if (!sessions || typeof sessions.open !== "function") return;
        // The forked session may not be in the client list snapshot yet (the
        // host attach/archive round-trip is async) and sessions.open throws
        // "unknown session" for ids missing from the list. Retry until the
        // list refresh lands so the UI actually switches to the new session.
        let attempts = 0;
        const tryOpen = () => {
          attempts += 1;
          try {
            ...
            return;
          } catch { /* not listed yet — keep waiting for the refresh */ }
          if (attempts < 15) {
            setTimeout(tryOpen, 400);
          } else if (typeof location !== "undefined" && typeof location.reload === "function") {
            // The list never refreshed; force a reload so the user sees the
            // new session state instead of the stale view.
            location.reload();
          }
        };
        tryOpen();
      ...
      // Editable-blocks cache shared by the assistant slot and the user injector.
      let dshmEditMessages = [];
      const dshmEditListeners = new Set();
      // Claimed eventSeqs per session, persisted across injector rebuilds so a
      // refresh never re-claims rows (repeated identical texts stay aligned).
      const dshmClaimedBySession = new Map();
      function setDshmEditMessages(list) {
        dshmEditMessages = Array.isArray(list) ? list : [];
        for (const fn of Array.from(dshmEditListeners)) fn();
      }
      function subscribeDshmEdit(fn) { dshmEditListeners.add(fn); return () => { dshmEditListeners.delete(fn); }; }
      function useDshmEditMessages() {
        return react.useSyncExternalStore(subscribeDshmEdit, () => dshmEditMessages, () => dshmEditMessages);
      }
      ...
      function mountEditor(block, sessionId) {
        ...
        save.addEventListener("click", () => {
          if (done) return;
          save.disabled = true;
          postMessageEdit({ action: "edit", sessionId, eventSeq: block.eventSeq, blockIndex: block.blockIndex, text: input.value }).then((res) => {
            if (res && res.sessionId) { close(); openSession(res.sessionId); }
            else {
              save.disabled = false;
              if (res && res.error && typeof alert === "function") alert(res.error);
            }
          });
        });
      ...
      // Assistant messages: React slot occupant with edit + delete buttons.
      function stripMdText(s) {
        return s.replace(/[`*_~>#|]/g, "").replace(/^\s*[-*+•·]\s+/gm, "").replace(/^\s*\d+[.、)]\s+/gm, "").replace(/(?:^|\s)[-*+•·]\s+/g, " ").replace(/(?:^|\s)\d+[.、)]\s+/g, " ").replace(/\s+/g, " ");
      }
      function plainText(s) {
        return (s || "")
          ...
          .replace(/!\[[^\]]*\]\([^)]*\)/g, " ")
          .replace(/\[([^\]]*)\]\([^)]*\)/g, "$1")
          // List markers render differently in the DOM (-/* → •, "1." → •),
          // and whitespace folding moves them mid-line, so strip markers in
          // ANY position from BOTH the API text and the DOM text.
          .replace(/^\s*[-*+•·]\s+/gm, "")
          .replace(/^\s*\d+[.、)]\s+/gm, "")
          .replace(/(?:^|\s)[-*+•·]\s+/g, " ")
          .replace(/(?:^|\s)\d+[.、)]\s+/g, " ")
      ...
      // All messages: debounced DOM injection. User message rows contain the
      // text in their own node; assistant rows live in a turn-tail node, so we
      // read the previous assistant-step sibling for their text.
      function injectMessageActions(messages, sessionId, onMiss) {
        let disposed = false;
        let timer = null;
        // Claims map eventSeq -> row node. A claim only counts while its row is
        // still in the document: React rebuilds rows on session switches, and
        // the fresh row must be able to re-claim its message (a persistent
        // used-set kept blocking it forever).
        let claimed = dshmClaimedBySession.get(sessionId);
        if (claimed === undefined) {
          claimed = new Map();
          dshmClaimedBySession.set(sessionId, claimed);
          if (dshmClaimedBySession.size > 8) {
            const oldest = dshmClaimedBySession.keys().next().value;
            if (oldest !== undefined) dshmClaimedBySession.delete(oldest);
          }
        }
        const sync = () => {
          if (disposed) return;
          let preparedForThisSync = null;
          // Rows flagged as no-match may have become matchable since (a turn
          // closed, a message finished, the API caught up): re-try everything
          // on every injection pass.
          const allRows = document.querySelectorAll('.p-xYUq_actions, [class$="_actions"]');
          for (const row of allRows) delete row.__dshmNoMatch;
          const rows = allRows;
          for (const row of rows) {
            if (row.__dshmEditInjected === true) continue;
            // Only rows inside a chat flow item count; the broad class selector
            // also matches sidebar/header rows whose ancestor text contains
            // message text and would steal the claim below.
            const flowItem = typeof row.closest === "function" ? row.closest('[data-chat-flow-kind]') : null;
            if (flowItem === null || !flowItem.dataset || typeof flowItem.dataset.chatFlowKind !== "string") continue;
            const flowKind = flowItem.dataset.chatFlowKind;
            if (flowKind !== "user" && flowKind !== "turn-tail") continue;
            let hay = (row.parentElement && row.parentElement.parentElement ? row.parentElement.parentElement.textContent : "").trim();
            if (flowKind === "turn-tail") {
              let sib = flowItem.previousElementSibling;
              let guard = 0;
              // Tool-calling turns interleave EMPTY assistant messages, so the
              // nearest assistant-step may have no text at all. Collect every
              // assistant-step with text; the longest-first matcher below picks
              // the final reply out of the merged hay.
              const texts = [];
              while (sib && guard < 32) {
                if (sib.dataset && sib.dataset.chatFlowKind === "assistant-step") {
              ...
              if (texts.length > 0) hay = texts.join(" ");
            }
            if (hay.length === 0) continue;
            const textNorm = hay.replace(/\s+/g, " ");
            const flatHay = plainText(hay);
            // IDENTITY INDEX FIRST: dsh stamps every chat row with
            // data-chat-anchor-key = "<kind-length>:<kind><id>" — user rows
            // carry the message id and turn-tail rows the turn number. Bind
            // by identity so markdown rendering differences can never
            // mis-bind a button ("你好" must never bind to "你好！我是…").
            let match;
            const anchorKey = flowItem && flowItem.dataset
              ? (flowItem.dataset.chatAnchorKey || flowItem.dataset.chatFlowKey || "")
              : "";
            if (typeof anchorKey === "string" && anchorKey.length > 0) {
              // key format: "<kind-length>:<kind><id>" — slice past the length
              // marker and the kind (kind.length === marker) to get the id.
              const keyHead = anchorKey.match(/^(\d+):/);
              let identity = "";
              if (keyHead !== null) {
                const kindLen = Number(keyHead[1]);
                identity = anchorKey.slice(String(kindLen).length + 1 + kindLen);
              }
              if (flowKind === "turn-tail") {
                const turn = Number(identity);
                if (Number.isFinite(turn)) {
                  const pool = messages.filter((m) => m.kind === "assistant.response" && m.turn === turn);
                  if (pool.length > 0) match = pool[pool.length - 1];
                }
              } else if (identity.length > 0) {
                match = messages.find((m) => m.kind === "user" && m.id === identity);
              }
            }
            if (match === undefined) {
              // Fallback: fuzzy text matching for hosts without anchor keys.
              // KIND FILTER FIRST: an AI row can only host assistant actions
              // and a user row only user actions.
              const rowPool = flowKind === "turn-tail"
                ? messages.filter((m) => m.kind === "assistant.response")
                : messages.filter((m) => m.kind === "user");
              // Lazily precompute per-message normalized text once per pass so
              // long replies are not re-regexed for every row.
              if (preparedForThisSync === null) {
                preparedForThisSync = new Map();
                for (const msg of messages) {
                  const msgNorm = (msg.text || "").replace(/\s+/g, " ").trim();
                  preparedForThisSync.set(msg.eventSeq, {
                    norm: msgNorm,
                    flat: plainText(msg.text),
                    strip: msgNorm.length > 0 ? stripMdText(msgNorm) : "",
                  });
                }
              }
              const candidates = rowPool.filter((m) => {
                const pre = preparedForThisSync.get(m.eventSeq);
                if (pre === undefined || pre.norm.length === 0) return false;
                if (textNorm.includes(pre.norm)) return true;
                if (pre.flat.length > 0 && flatHay.includes(pre.flat)) return true;
                const loosePrefix = pre.strip.slice(0, 30);
                return loosePrefix.length >= 4 && stripMdText(textNorm).includes(loosePrefix);
              });
              if (candidates.length === 1) {
                match = candidates[0];
              } else if (candidates.length > 1) {
                const free = candidates.filter((m) => {
                  const claimedRow = claimed.get(m.eventSeq);
                  return claimedRow === undefined || claimedRow.isConnected !== true;
                });
            ...
            if (match === undefined) {
              // No editable message matches this row: flag it once so the
              // watcher does not loop-refresh, but ONLY when we actually had a
              // message list (an empty list means the fetch failed — retry).
              if (messages.length > 0 && row.__dshmNoMatch !== true) {
                row.__dshmNoMatch = true;
              }
              continue;
            }
            claimed.set(match.eventSeq, row);
            ...
            const placeBtn = (btn) => {
              // NEVER insert between React-managed children: React reconciles
              // the row by child index, and a foreign node in the middle makes
              // it rebuild the row on every render (render storm -> OOM).
              // Always append at the END. For AI rows the stats span is kept
              // visually last via flex `order` (set once below), so the icons
              // still sit between the copy/branch cluster and the stats text.
              row.appendChild(btn);
            };
            if (flowKind === "turn-tail" && row.__dshmStatsOrdered !== true) {
              const last = row.lastElementChild;
              if (last !== null && (last.tagName || "").toUpperCase() === "SPAN" && last.style !== undefined && last.style !== null) {
                last.style.order = "1";
                row.__dshmStatsOrdered = true;
              }
            }
            placeBtn(mkBtn(EDIT_PATH, t("msgEdit"), () => mountEditor(match, sessionId)));
            placeBtn(mkBtn(TRASH_PATH, t("msgDelete"), () => {
              const sure = (typeof window !== "undefined" && typeof window.confirm === "function") ? window.confirm(t("msgDeleteConfirm")) : true;
              if (!sure) return;
              postMessageEdit({ action: "delete", sessionId, eventSeq: match.eventSeq }).then((res) => {
                if (res && res.sessionId) openSession(res.sessionId);
                else if (res && res.error && typeof alert === "function") alert(res.error);
              });
            }));
            if (match.kind === "user") {
              placeBtn(mkBtn(REFRESH_PATH, t("msgRegenerate"), () => {
                postMessageEdit({ action: "reroll", sessionId, eventSeq: match.eventSeq }).then((res) => {
                  if (res && res.sessionId) openSession(res.sessionId);
                  else if (res && res.error && typeof alert === "function") alert(res.error);
                });
              }));
        ...
        // Initial pass only: later rows are handled by the global watcher
        // (session-list current + DOM scan + poll), which rebuilds this
        // injector when it needs to. No per-injector MutationObserver here —
        // nested document-wide observers were the OOM feedback loop.
      ...
      function WebSearchCard() {
        ...
        const TYPE_OPTIONS = [
          { value: "exa", label: t("searchModelTypeExa") },
          { value: "brave", label: t("searchModelTypeBrave") },
          { value: "bing", label: t("searchModelTypeBing") },
          { value: "tavily", label: t("searchModelTypeTavily") },
          { value: "firecrawl", label: t("searchModelTypeFirecrawl") },
          { value: "deepseek", label: t("searchModelTypeDeepseek") },
          { value: "you", label: t("searchModelTypeYou") },
          { value: "serper", label: t("searchModelTypeSerper") },
          { value: "serpapi", label: t("searchModelTypeSerpapi") },
          { value: "kagi", label: t("searchModelTypeKagi") },
          { value: "searxng", label: t("searchModelTypeSearxng") },
          { value: "bocha", label: t("searchModelTypeBocha") },
        ];
        const currentTypeLabel = TYPE_OPTIONS.find((o) => o.value === type)?.label || type;
        const dshmSearchTypeBrands = {
          exa: "Exa", brave: "Brave", bing: "Bing", tavily: "Tavily", firecrawl: "Firecrawl",
          deepseek: "DeepSeek", you: "You.com", serper: "Serper", serpapi: "SerpApi",
          kagi: "Kagi", searxng: "SearXNG", bocha: "Bocha",
        };
        ...
        const form = react.createElement("div", { className: "dshm-search-form" },
          ...
          react.createElement("div", { className: "dshm-search-type-wrap" },
            react.createElement("button", {
              type: "button",
              className: "dshm-search-type-btn" + (typeOpen ? " dshm-open" : ""),
              "aria-expanded": typeOpen ? "true" : "false",
              "aria-label": t("searchModelType"),
              onClick: () => setTypeOpen(!typeOpen),
            },
              react.createElement("span", { className: "dshm-search-type-btn-label" }, currentTypeLabel),
              react.createElement("span", { className: "dshm-search-type-chevron" },
            ...
            typeOpen ? react.createElement("div", { className: "dshm-search-type-menu" },
              TYPE_OPTIONS.map((o) =>
                react.createElement("button", {
                  type: "button",
                  key: o.value,
                  className: "dshm-search-type-option",
                  onClick: () => { setType(o.value); setTypeOpen(false); },
                },
                  react.createElement("span", { className: "dshm-search-type-option-name" }, o.label),
                  o.value === type ? react.createElement("span", { className: "dshm-search-type-option-check" }, "✓") : null))) : null),
        ...
        const header = react.createElement("button", {
          ...
          react.createElement("span", { className: "dshm-plugin-card-chevron" + (open ? " dshm-plugin-card-chevron-open" : "") },
            react.createElement("svg", { width: 14, height: 14, viewBox: "0 0 14 14", fill: "none", "aria-hidden": true },
              react.createElement("path", { d: "M11.8486 5.5L11.4238 5.92383L8.69727 8.65137C8.44157 8.90706 8.21562 9.13382 8.01172 9.29785C7.79912 9.46883 7.55595 9.61756 7.25 9.66602C7.08435 9.69222 6.91565 9.69222 6.75 9.66602C6.44405 9.61756 6.20088 9.46883 5.98828 9.29785C5.78438 9.13382 5.55843 8.90706 5.30273 8.65137L2.57617 5.92383L2.15137 5.5L3 4.65137L3.42383 5.07617L6.15137 7.80273C6.42595 8.07732 6.59876 8.24849 6.74023 8.3623C6.87291 8.46904 6.92272 8.47813 6.9375 8.48047C6.97895 8.48703 7.02105 8.48703 7.0625 8.48047C7.07728 8.47813 7.12709 8.46904 7.25977 8.3623C7.40124 8.24849 7.57405 8.07732 7.84863 7.80273L10.5762 5.07617L11 4.65137L11.8486 5.5Z", fill: "currentColor" }))));
        return react.createElement("li", { className: "dshm-plugin-card" + (open ? " dshm-plugin-card-open" : "") },
          header,
            ...
            status === "ready" && models.map((m) => react.createElement("div", { key: m.id, className: "dshm-search-row" },
              react.createElement("div", { className: "dshm-search-row-info" },
                ...
                react.createElement("div", { className: "dshm-search-row-model" },
                  (dshmSearchTypeBrands[m.type] || m.type) + " · " + (m.model || "") +
      ...
      ctx.effect(() => {
        ...
        // model-select popup renders provider group titles. Also auto-refresh
        // every 60s while the observer is active (popup likely stays open).
        fetchBalances(true);
        let refreshTimer = setInterval(() => {
          // Only refresh if the model-select popup is actually visible
          if (typeof document !== "undefined" && document.querySelector("._7KE1Ra_menu, ._7KE1Ra_groupTitle")) {
            fetchBalances(false);
          }
        }, 60000);
        let balMo = null;
            ...
            for (const rec of records) {
              for (const node of rec.addedNodes) {
                ...
                if (node.matches && (node.matches("._7KE1Ra_groupTitle") ||
                  ...
                  fetchBalances(false);
                  if (typeof requestAnimationFrame !== "undefined") {
                    requestAnimationFrame(() => injectBalances(fetchFailed ? "error" : "done"));
                  }
                  return;
                }
              }
      ...
      ctx.effect(() => {
        ...
        return () => { off1(); off2(); off2b(); off3(); off4(); off5(); off5b(); off5c(); off5d(); off6(); off7(); off8(); off9(); off10(); };
      }, "ui-mobile: slots");

      // ---------- global message-actions injector ----------
      // Lives in apply() instead of a slot component so it survives session
      // switches, slot re-renders and any component-lifecycle surprise. Three
      // triggers re-pull editable blocks: the global session list's current id
      // changing (session switch), a DOM row without buttons appearing (new
      // message), and a slow polling fallback.
      ctx.effect(() => {
        ...
        const refresh = () => {
          ...
          fetch("/api/dshm/message-edit?sessionId=" + encodeURIComponent(sessionId), { cache: "no-store" })
            ...
            .then((d) => {
              ...
              if (messages.length === lastMessagesLength) missStreak += 1;
              else { lastMessagesLength = messages.length; missStreak = 0; }
              // Cheap change detection: skip the full re-injection pass when
              // the editable list is unchanged (e.g. redundant onMiss pings).
              const signature = messages.length + ":" + (messages.length > 0 ? messages[messages.length - 1].eventSeq : "");
              if (signature !== lastSignature) {
                lastSignature = signature;
                setDshmEditMessages(messages);
                if (cleanup !== null) cleanup();
                cleanup = null;
                if (messages.length > 0) cleanup = injectMessageActions(messages, sessionId, onMiss);
              }
            })
            .catch(() => {});
        };
        const onMiss = () => {
          ...
          const now = Date.now();
          const minGap = missStreak >= 3 ? 10000 : 1000;
          if (now - lastFetchAt < minGap) return;
          if (missTimer !== null) clearTimeout(missTimer);
          missTimer = setTimeout(() => { missTimer = null; refresh(); }, 300);
        };
        const scan = () => {
          if (disposed || typeof document.querySelectorAll !== "function") return;
          const now = Date.now();
          if (now - lastScanAt < 500) return;
          lastScanAt = now;
          const rows = document.querySelectorAll('.p-xYUq_actions, [class$="_actions"]');
          for (const row of rows) {
            if (row.__dshmEditInjected === true || row.__dshmNoMatch === true) continue;
            const flowItem = typeof row.closest === "function" ? row.closest('[data-chat-flow-kind]') : null;
            if (flowItem === null || !flowItem.dataset || typeof flowItem.dataset.chatFlowKind !== "string") continue;
            const flowKind = flowItem.dataset.chatFlowKind;
            if (flowKind !== "user" && flowKind !== "turn-tail") continue;
            // A buttonless row means the injector has not run yet or the
            // session gained a message. Never flag the row here — flagging
            // races the injector and can hide buttons forever. Just request a
            // refresh; onMiss is rate-limited.
            onMiss();
            return;
          }
        };
        const sync = () => {
          if (disposed || sessionsSvc === undefined) return;
          const snap = sessionsSvc.list.getSnapshot();
          const id = snap === null || snap === undefined ? undefined : snap.current;
        ...
        const start = (svc) => {
          sessionsSvc = svc;
          offList = svc.list.subscribe(sync);
          sync();
          // Polling is only a safety net behind the MutationObserver; a long
          // interval avoids periodic full-document scans on stable pages.
          pollTimer = setInterval(scan, 10000);
          // Debounce DOM changes: a burst (streaming chunks, React commits)
          // collapses into ONE scan after the tree settles, so a freshly
          // rendered row is never scanned "too early" and then missed because
          // of the 500ms rate limit.
          observer = new MutationObserver(() => {
            if (scanDebounce !== null) clearTimeout(scanDebounce);
            scanDebounce = setTimeout(() => { scanDebounce = null; scan(); }, 250);
          });
          observer.observe(document.body || document.documentElement, { childList: true, subtree: true });
        };
        const tryStart = () => {
          if (disposed) return;
          const svc = dshmCtx !== null && typeof dshmCtx.get === "function" ? dshmCtx.get("sessions") : undefined;
          if (svc === undefined || svc === null || svc.list === undefined || svc.list === null
            || typeof svc.list.subscribe !== "function" || typeof svc.list.getSnapshot !== "function") {
            retryTimer = setTimeout(tryStart, 1000);
            return;
          }
          start(svc);
        };
      ...
      }, "ui-mobile: message-edit global injector");
    }

...
function BackgroundRow() {
      ...
      function injectMessageActions(messages, sessionId, onMiss) {
        ...
        if (claimed === undefined) { claimed = new Map(); dshmClaimedBySession.set(sessionId, claimed); }
        const sync = () => {
          if (disposed) return;
          ...
          for (const row of rows) {
            ...
            if (match === undefined) {
              ...
              const candidates = rowPool.filter((m) => {
                ...
                if (textNorm.includes(norm)) return true;
                const flat = plainText(m.text);
                if (flat.length > 0 && flatHay.includes(flat)) return true;
                const stripMd = (s) => s.replace(/[`*_~>#|]/g, "").replace(/^\s*[-*+•·]\s+/gm, "").replace(/^\s*\d+[.、)]\s+/gm, "").replace(/(?:^|\s)[-*+•·]\s+/g, " ").replace(/(?:^|\s)\d+[.、)]\s+/g, " ").replace(/\s+/g, " ");
                const loosePrefix = stripMd(norm).slice(0, 30);
                return loosePrefix.length >= 4 && stripMd(textNorm).includes(loosePrefix);
              });

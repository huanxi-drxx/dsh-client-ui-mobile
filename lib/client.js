...
function BackgroundRow() {
        ...
        fileInputEl.onchange = async () => {
          ...
          try {
            ...
            if (fileInputActions && typeof fileInputActions.setDraft === "function") {
              // On success: show the bare path only — no auto-send, so the user
              // can add instructions for the file before hitting send.
              fileInputActions.setDraft(res.ok && j && typeof j.path === "string"
                ? j.path
                : "⚠️ " + t("uploadFailed"));
            }
          } catch (_e) {
      ...
      ctx.effect(() => {
        ...
        return () => { off1(); off2(); off2b(); off3(); off4(); off5(); off5b(); off5c(); off5d(); off6(); off7(); };
      }, "ui-mobile: slots");
    }

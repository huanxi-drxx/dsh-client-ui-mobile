...
function BackgroundRow() {
      ...
      function MessageEditInjector(props) {
        ...
        const sessionId = typeof props.useSession === "function"
          ...
          : props.sessionId;
        // New user/AI messages always add chat nodes, so a changing order
        // length is a reliable "a message was sent" signal to re-pull blocks.
        const chatOrderLen = typeof props.useSession === "function"
          ? props.useSession((s) => (s && s.chat && Array.isArray(s.chat.order) ? s.chat.order.length : 0))
          : 0;
        react.useEffect(() => {
          ...
          // A row we cannot match appeared — the session gained a new message.
          // Debounce, cap the rate, and back off while refreshes change nothing
          // (e.g. while the assistant is still streaming its reply).
          const onMiss = () => {
            if (!current) return;
            const now = Date.now();
          ...
          // Refresh immediately on mount and whenever the chat node count
          // changes (a user or AI message was just sent). The miss fallback
          // still covers rows that appear without an order bump.
          refresh();
          return () => {
            current = false;
            if (missTimer !== null) clearTimeout(missTimer);

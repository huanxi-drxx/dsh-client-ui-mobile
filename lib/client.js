...
function BackgroundRow() {
      ...
      function startMessageEditInjection(messages, sessionId) {
        ...
        const sync = () => {
          const rows = document.querySelectorAll('[class*="actions"]');
          for (const row of rows) {
            ...
            const matching = [...new Set(messages
              .filter((m) => m.text.length > 0 && text.includes(m.text.slice(0, 24)))
              .map((m) => m.eventSeq))];
      ...
      function MessageEditInjector(props) {
        ...
        const sessionId = typeof props.useSession === "function"
          ...
          : props.sessionId;
        if (typeof console !== "undefined") console.log("[dshm] MessageEditInjector render, sessionId=", sessionId, "has useSession=", typeof props.useSession);
        react.useEffect(() => {
          if (typeof console !== "undefined") console.log("[dshm] MessageEditInjector effect, sessionId=", sessionId);
          ...
          fetch("/api/dshm/message-edit?sessionId=" + encodeURIComponent(sessionId), { cache: "no-store" })
            ...
            .then((d) => {
              ...
              if (typeof console !== "undefined") console.log("[dshm] message-edit session", sessionId, "editable:", messages.length);
              if (messages.length > 0) observer = startMessageEditInjection(messages, sessionId);
            })
            .catch((e) => { if (typeof console !== "undefined") console.log("[dshm] message-edit fetch error", e && e.message); });

window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    .dshm-search-model-row:active { background: var(--dsw-alias-interactive-bg-hover, rgb(0 0 0 / 6%)); }
    .dshm-search-model-icon { flex: none; }
    .dshm-search-model-label { flex: 1; min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
    .dshm-search-model-chevron { flex: none; color: var(--dsw-alias-label-tertiary, #81858c); }
    ...
    .dshm-plugin-card-chevron { flex: none; color: var(--dsw-alias-label-tertiary, #81858c); }
...
function BackgroundRow() {
      ...
      // Inject a "Search model: <label> ▾" row at the bottom of the model menu.
      function ensureSearchModelRow() {
        ...
        row.type = "button";
        row.className = "dshm-search-model-row";
        row.setAttribute("data-dshm-search-row", "1");
        row.setAttribute("aria-haspopup", "menu");
        const icon = document.createElement("span");
        icon.className = "dshm-search-model-icon";
        icon.textContent = "🔍";
        const txt = document.createElement("span");
        txt.className = "dshm-search-model-label";
        txt.textContent = t("searchModel") + ": " + (label ? (label.label || label.model) : "…");
      ...
      function refreshSearchModelRowText() {
        ...
        if (!row) return;
        const label = searchModelsCache.models.find((m) => m.id === searchModelsCache.current);
        const txt = row.querySelector(".dshm-search-model-label");
        if (txt) txt.textContent = t("searchModel") + ": " + (label ? (label.label || label.model) : "…");
      }
      ...
      function WebSearchCard() {
        ...
        const addModel = () => {
          ...
          postSearchModels({ models: next }).then((ok) => {
            if (ok) { setLabel(""); setModelId(""); setBaseURL(""); load(); }
        ...
        return react.createElement("li", { className: "dshm-plugin-card" },
          react.createElement("button", {
            ...
            react.createElement("span", { className: "dshm-plugin-card-chevron" }, open ? "▴" : "▾")),
          open ? react.createElement("div", { className: "dshm-plugin-card-body" },
            ...
            status === "ready" && models.map((m) => react.createElement("div", { key: m.id, className: "dshm-search-row" },
              react.createElement("div", { className: "dshm-search-row-info" },
                ...
                react.createElement("div", { className: "dshm-search-row-model" }, m.model)),
      ...
      // ---------- Tools settings page (description + parameters per tool) ----------
      function ToolsPage() {
        useLocaleTick();
      ...
      ctx.effect(() => {
        ...
        const off10 = slots.inject("settings.plugin.item", () => slots.register({ name: "settings.plugin.item", id: "web-search", order: 20, label: () => t("webSearchTitle") }, WebSearchCard));

        ...
        const sync = () => {
          ...
          for (const row of rows) {
            ...
            if (flowKind === "turn-tail") {
              ...
              let guard = 0;
              while (sib && guard < 16) {
                if (sib.dataset && sib.dataset.chatFlowKind === "assistant-step") { hay = sib.textContent; break; }
            ...
            const match = messages.find((m) => {
              ...
              if (claimedRow !== undefined && claimedRow.isConnected === true) return false;
              const needle = (m.text || "").replace(/\s+/g, " ").trim().slice(0, 24);
              if (needle.length > 0 && textNorm.includes(needle)) return true;
              const flat = plainText(m.text).slice(0, 20);
              return flat.length > 0 && flatHay.includes(flat);
            });
            if (match === undefined) {
              // Uninjected row whose text matches nothing we know: the session
              // probably gained a new message. Ask the owner to refresh once.
            ...
            placeBtn(mkBtn(TRASH_PATH, t("msgDelete"), () => {
              ...
              if (!sure) return;
              postMessageEdit({ action: "delete", sessionId, eventSeq: match.eventSeq }).then((newId) => { if (newId) openSession(newId); });
            }));
            if (match.kind === "user") {

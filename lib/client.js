...
function BackgroundRow() {
      ...
      const dshmEditUsedBySession = new Map();
      ...
      function injectMessageActions(messages, sessionId, onMiss) {
        let disposed = false;
        let timer = null;
        let used = dshmEditUsedBySession.get(sessionId);
        if (used === undefined) { used = new Set(); dshmEditUsedBySession.set(sessionId, used); }
        const sync = () => {
          if (disposed) return;
          ...
          for (const row of rows) {
            ...
            const match = messages.find((m) => {
              if (used.has(m.eventSeq)) return false;

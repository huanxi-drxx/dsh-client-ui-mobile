window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /* Search-model row always renders at the BOTTOM of the model menu, even
       though the menu's groups load asynchronously after we inject. */
    .dshm-search-model-row { order: 99; }
    .dshm-search-row {
      display: flex;
      align-items: center;
      gap: 8px;
...
function BackgroundRow() {
      ...
      // The row appears ONLY in menus that actually list model groups; other
      // popups that reuse the same menu class (e.g. the reasoning-level
      // popupSelect) get no row and any stale row is removed.
      function ensureSearchModelRow() {
        ...
        if (!menu.querySelector("._7KE1Ra_groups, ._7KE1Ra_groupTitle")) {
        ...
        if (typeof MutationObserver !== "undefined" && typeof document !== "undefined") {
          smMo = new MutationObserver((records) => {
            ...
            for (const rec of records) {
              for (const node of rec.addedNodes) {
                if (node.nodeType !== 1) continue;
                if (node.matches && (node.matches("._7KE1Ra_menu, ._7KE1Ra_groups, ._7KE1Ra_groupTitle") ||
                    (node.querySelector && node.querySelector("._7KE1Ra_menu, ._7KE1Ra_groups, ._7KE1Ra_groupTitle")))) {
                  relevant = true;
                  break;
                }
              }

...
function BackgroundRow() {
      ...
      function injectMessageActions(messages, sessionId) {
        let disposed = false;
        let timer = null;
        const used = new Set();
        const sync = () => {
        ...
        react.useEffect(() => {
          ...
          fetch("/api/dshm/message-edit?sessionId=" + encodeURIComponent(sessionId), { cache: "no-store" })
            ...
            .then((d) => {
              ...
              if (messages.length > 0) cleanup = injectMessageActions(messages, sessionId);
            })
            .catch(() => {});
          return () => { current = false; if (cleanup !== null) cleanup(); };
        }, [sessionId]);
        return null;
      }

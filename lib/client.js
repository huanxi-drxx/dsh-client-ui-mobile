...
function BackgroundRow() {
      ...
      // Inject a "Search model" row into the model-select menu. The menu is a
      // multi-pane container: the root pane holds "Model"/"Reasoning level"
      // cells, the model pane lists model groups, and the effort pane lists
      // reasoning levels. The row shows on the root and model panes (has cells
      // or group titles) and is REMOVED on the effort pane.
      function ensureSearchModelRow() {
        ...
        for (const menu of menus) {
          ...
          const show = menu.querySelector("._7KE1Ra_cell, ._7KE1Ra_groupTitle") !== null;
          if (!show) {
            if (existing) { existing.remove(); menu.style.paddingBottom = ""; }
            continue;
          }
          ...
          name.className = "dshm-search-model-name _7KE1Ra_modelName";
      ...
      function SearchModelSheet() {
        ...
        react.useEffect(() => {
          if (!open) return;
          let current = true;
          // Cover the open model menu: same spot, same width, so switching to
          // the search-model panel visually replaces it (like the effort pane).
        ...
        if (!open) return null;
        const sheetStyle = pos !== null
          ? { position: "fixed", left: pos.left + "px", top: pos.top + "px", width: pos.width + "px" }
          : { position: "fixed", left: "50%", bottom: "10px", transform: "translateX(-50%)", width: "min(260px, 100vw - 32px)" };
        return react.createElement(react.Fragment, null,
          react.createElement("div", { className: "dshm-picker-backdrop" }),
          react.createElement("div", { className: "dshm-picker-sheet dshm-search-model-sheet", role: "menu", style: sheetStyle },
            ...
            list.models.map((m) => react.createElement("button", {
              key: m.id,
              type: "button",
              className: "dshm-picker-item _7KE1Ra_option" + (m.id === list.current ? " _7KE1Ra_selected" : ""),
              role: "menuitemradio",
              ...
              react.createElement("span", { className: "dshm-search-model-name _7KE1Ra_modelName" }, m.label || m.model),
              m.id === list.current ? react.createElement("span", { className: "_7KE1Ra_check" },
                react.createElement("svg", { width: 16, height: 16, viewBox: "0 0 16 16", fill: "none", stroke: "currentColor", strokeWidth: 1.5, strokeLinecap: "round", "aria-hidden": true },
                  react.createElement("path", { d: "M3.5 8.5l3 3 6-7" }))) : null))));
      }
      ...
      function WebSearchCard() {
        ...
        const addModel = () => {
          const name = (type === "exa" || type === "brave" || type === "bing" ? (label || "").trim() : (modelId || "").trim());
            ...
            status === "ready" && models.map((m) => react.createElement("div", { key: m.id, className: "dshm-search-row" },
              react.createElement("div", { className: "dshm-search-row-info" },
                ...
                react.createElement("div", { className: "dshm-search-row-model" },
                  (m.type === "exa" ? "Exa" : m.type === "brave" ? "Brave" : m.type === "bing" ? "Bing" : "DeepSeek") + " · " + (m.model || "") +

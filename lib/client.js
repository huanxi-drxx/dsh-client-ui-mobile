      ...
      function injectMessageActions(messages, sessionId, onMiss) {
        ...
        const sync = () => {
          ...
          for (const row of rows) {
            ...
            const placeBtn = (btn) => {
              // AI rows: sit between the icon cluster (copy/branch) and the
              // trailing stats span (time + run stats), not after the stats.
              if (flowKind === "turn-tail" && typeof row.insertBefore === "function") {
                const last = row.lastElementChild;
                if (last !== null && (last.tagName || "").toUpperCase() === "SPAN") { row.insertBefore(btn, last); return; }
              }
              row.appendChild(btn);
            };

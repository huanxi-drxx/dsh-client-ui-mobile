...
function BackgroundRow() {
      ...
      function useDshmEditMessages() {
        return react.useSyncExternalStore(subscribeDshmEdit, () => dshmEditMessages);
      }
      ...
      function AssistantMessageActions(props) {
        useLocaleTick();
        const list = useDshmEditMessages();
        const blocks = list.filter((m) => m.messageId === props.messageId && m.kind !== "user");
        if (blocks.length === 0) return null;
        const b = blocks[0];
        return react.createElement(react.Fragment, null,
          react.createElement("button", {
            type: "button",
            className: "dshm-msg-action",
            "aria-label": t("msgEdit"),
            title: t("msgEdit"),
            onClick: () => mountEditor(b, props.sessionId),
          }, msgSvgIcon(EDIT_PATH)),
          react.createElement("button", {
            type: "button",
            className: "dshm-msg-action",
            "aria-label": t("msgDelete"),
            title: t("msgDelete"),
            onClick: () => {
              const sure = (typeof window !== "undefined" && typeof window.confirm === "function") ? window.confirm(t("msgDeleteConfirm")) : true;
              if (!sure) return;
              postMessageEdit({ action: "delete", sessionId: props.sessionId, eventSeq: b.eventSeq }).then((newId) => { if (newId) openSession(newId); });
            },
          }, msgSvgIcon(TRASH_PATH)));
      }

      // User messages: debounced DOM injection (edit/delete/regenerate).
      function injectUserActions(messages, sessionId) {
        let disposed = false;
        let timer = null;
        const userBlocks = messages.filter((m) => m.kind === "user");
        const sync = () => {
          if (disposed) return;
          ...
          for (const row of rows) {
            ...
            const match = userBlocks.find((m) => {
              const needle = (m.text || "").replace(/\s+/g, " ").trim().slice(0, 24);
              return needle.length > 0 && textNorm.includes(needle);
            });
        ...
        react.useEffect(() => {
          ...
          fetch("/api/dshm/message-edit?sessionId=" + encodeURIComponent(sessionId), { cache: "no-store" })
            ...
            .then((d) => {
              ...
              if (messages.length > 0) cleanup = injectUserActions(messages, sessionId);
            })
            .catch(() => {});
      ...
      ctx.effect(() => {
        ...
        const off13 = slots.inject("conversation.chat.assistant-actions", () => slots.register({ name: "conversation.chat.assistant-actions", id: "dshm-assistant-edit", order: 20 }, AssistantMessageActions));
        return () => { off1(); off2(); off2b(); off3(); off4(); off5(); off5b(); off5c(); off5d(); off6(); off7(); off8(); off9(); off10(); off12(); off13(); };
      }, "ui-mobile: slots");
    }

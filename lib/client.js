        ...
        const sync = () => {
          ...
          for (const row of rows) {
            ...
            const matching = [...new Set(messages
              .filter((m) => {
                // Primary: the exact text (works for plain user messages).
                const needle = (m.text || "").replace(/\s+/g, " ").trim().slice(0, 24);
                if (needle.length > 0 && textNorm.includes(needle)) return true;
                // Fallback: markdown-stripped (assistant replies render markdown).
                const flatNeedle = plainText(m.text).slice(0, 20);
                return flatNeedle.length > 0 && plainText(text).includes(flatNeedle);
              })
              .map((m) => m.eventSeq))];

...
// You.com: GET https://api.ydc-index.io/search?q=... with X-API-Key header.
async function youSearch(model, apiKey, request, signal) {
  const base = String(model.baseURL || "https://api.ydc-index.io").replace(/\/$/, "");
  const count = Number.isFinite(Number(request.maxResults)) && Number(request.maxResults) > 0 ? Number(request.maxResults) : 5;
  const params = new URLSearchParams({ q: request.query, num_web_results: String(count) });
  const parsed = await fetchJson(`${base}/search?${params.toString()}`, {
    method: "GET",
    redirect: "error",
    headers: {
      "X-API-Key": apiKey,
  ...
  const results = parsed && Array.isArray(parsed.hits) ? parsed.hits : [];
  const sources = [];
  const seen = new Set();
  for (const item of results) {
    if (!item || typeof item.url !== "string" || item.url.length === 0 || seen.has(item.url)) continue;
    seen.add(item.url);
    const snippet = Array.isArray(item.snippets) && item.snippets.length > 0 && typeof item.snippets[0] === "string"
      ? item.snippets[0]
...
// Serper.dev: POST https://google.serper.dev/search, X-API-KEY, organic[]
async function serperSearch(model, apiKey, request, signal) {
  const base = String(model.baseURL || "https://google.serper.dev").replace(/\/$/, "");
  const count = Number.isFinite(Number(request.maxResults)) && Number(request.maxResults) > 0 ? Number(request.maxResults) : 5;
  const parsed = await fetchJson(base + "/search", {
    method: "POST",
    redirect: "error",
    ...
    body: JSON.stringify({ q: request.query, num: count }),
  }, signal);
  const results = parsed && Array.isArray(parsed.organic) ? parsed.organic : [];
  const sources = [];
  const seen = new Set();
  for (const item of results) {
    if (!item || typeof item.link !== "string" || item.link.length === 0 || seen.has(item.link)) continue;
    seen.add(item.link);
    sources.push({
      url: item.link,
      ...(typeof item.title === "string" && item.title.length > 0 ? { title: item.title } : {}),
...
// SerpApi: GET https://serpapi.com/search?q=&api_key=&num=
async function serpapiSearch(model, apiKey, request, signal) {
  const base = String(model.baseURL || "https://serpapi.com").replace(/\/$/, "");
  const count = Number.isFinite(Number(request.maxResults)) && Number(request.maxResults) > 0 ? Number(request.maxResults) : 5;
  const params = new URLSearchParams({ q: request.query, api_key: apiKey, num: String(count) });
  const parsed = await fetchJson(`${base}/search?${params.toString()}`, {
    method: "GET",
    redirect: "error",
    headers: {
  ...
  const results = parsed && Array.isArray(parsed.organic_results) ? parsed.organic_results : [];
  const sources = [];
  const seen = new Set();
  for (const item of results) {
    if (!item || typeof item.link !== "string" || item.link.length === 0 || seen.has(item.link)) continue;
...
// Kagi: GET https://kagi.com/api/v0/search?q=&limit=, Authorization: Bearer
async function kagiSearch(model, apiKey, request, signal) {
  const base = String(model.baseURL || "https://kagi.com/api/v0").replace(/\/$/, "");
  const count = Number.isFinite(Number(request.maxResults)) && Number(request.maxResults) > 0 ? Number(request.maxResults) : 5;
  const params = new URLSearchParams({ q: request.query, limit: String(count) });
  const parsed = await fetchJson(`${base}/search?${params.toString()}`, {
    method: "GET",
    redirect: "error",
    headers: {
...
// SearXNG (self-hosted): GET {base}/search?q=&format=json — no API key needed.
async function searxngSearch(model, apiKey, request, signal) {
  const base = String(model.baseURL || "").replace(/\/$/, "");
  if (base.length === 0) throw new Error("SearXNG requires a base URL (self-hosted instance)");
  const count = Number.isFinite(Number(request.maxResults)) && Number(request.maxResults) > 0 ? Number(request.maxResults) : 5;
  const params = new URLSearchParams({ q: request.query, format: "json", number_of_results: String(count) });
  const parsed = await fetchJson(`${base}/search?${params.toString()}`, {
    method: "GET",
    redirect: "error",
    headers: {
...
// Bocha (博查搜索): POST https://api.bochaai.com/v1/web-search, Authorization: Bearer
async function bochaSearch(model, apiKey, request, signal) {
  const base = String(model.baseURL || "https://api.bochaai.com/v1").replace(/\/$/, "");
  const count = Number.isFinite(Number(request.maxResults)) && Number(request.maxResults) > 0 ? Number(request.maxResults) : 5;
  const parsed = await fetchJson(base + "/web-search", {
    method: "POST",
    redirect: "error",
    headers: {
      "Authorization": `Bearer ${apiKey}`,
    ...
    body: JSON.stringify({ query: request.query, count, freshness: "noLimit", summary: false }),
  }, signal);
  const pages = parsed && parsed.data && parsed.data.webPages && Array.isArray(parsed.data.webPages.value) ? parsed.data.webPages.value : [];
  const sources = [];
  const seen = new Set();
  for (const item of pages) {
    if (!item || typeof item.url !== "string" || item.url.length === 0 || seen.has(item.url)) continue;
    seen.add(item.url);
    const snippet = typeof item.summary === "string" && item.summary.length > 0 ? item.summary : (typeof item.snippet === "string" ? item.snippet : "");
    const title = typeof item.title === "string" && item.title.length > 0 ? item.title : (typeof item.name === "string" ? item.name : "");
    sources.push({
      url: item.url,
...
function createSearchProvider(credentialsSvc) {
  return {
    ...
    async search(request, signal) {
      ...
      if (type !== "searxng" && !apiKey) throw new Error(`search has no API key configured for "${model.label || model.id}"`);
      ...
      if (type === "you") return youSearch(model, apiKey, request, signal);
      if (type === "serper") return serperSearch(model, apiKey, request, signal);
      if (type === "serpapi") return serpapiSearch(model, apiKey, request, signal);
      if (type === "kagi") return kagiSearch(model, apiKey, request, signal);
      if (type === "searxng") return searxngSearch(model, apiKey, request, signal);
      if (type === "bocha") return bochaSearch(model, apiKey, request, signal);
...
// User messages that appear BEFORE the first turn/start (dsh writes the
// opening user message outside any turn) are invisible to closedTurns. They
// are still real, editable messages, so surface them separately.
function orphanUserMessages(events, turns) {
  const accounted = new Set();
  for (const turn of turns) {
    if (turn.user !== undefined) accounted.add(turn.user.seq);
    for (const event of turn.assistants) accounted.add(event.seq);
  }
  const firstTurnStart = events.find((e) => e.type === "turn/start")?.seq;
  if (firstTurnStart === undefined) return [];
  return events.filter((e) => e.type === "user/message"
    && e.seq < firstTurnStart
    && !accounted.has(e.seq)
    && e.data && e.data.source && e.data.source.kind === "user");
}

// The final turn may still be open (the AI reply was stopped mid-way). Its
// user message is real and editable; surface it too, keyed by the open turn.
function openTurnUser(events, turns) {
  const lastStart = events.reduce((m, e) => (e.type === "turn/start" ? Math.max(m, e.seq) : m), -1);
  const lastEnd = events.reduce((m, e) => (e.type === "turn/end" ? Math.max(m, e.seq) : m), -1);
  if (lastStart === -1 || lastStart <= lastEnd) return undefined;
  const startEvent = events.find((e) => e.seq === lastStart);
  if (startEvent === undefined) return undefined;
  const turn = startEvent.data.turn;
  const accounted = new Set();
  for (const t of turns) {
    if (t.user !== undefined) accounted.add(t.user.seq);
    for (const e of t.assistants) accounted.add(e.seq);
  }
  const user = events.find((e) => e.type === "user/message" && e.seq > lastStart
    && e.data && e.data.source && e.data.source.kind === "user" && !accounted.has(e.seq));
  return user === undefined ? undefined : { turn, startSeq: lastStart, user };
}

function editableMessages(turns, events) {
  const result = [];
  for (const turn of turns) {
  ...
  for (const event of orphanUserMessages(events, turns)) {
    const texts = event.data.content.filter((b) => b.type === "text").map((b) => b.text);
    if (texts.length === 0) continue;
    result.push({
      key: `${event.seq}:all`,
      turn: -1,
  ...
  const open = openTurnUser(events, turns);
  if (open !== undefined) {
    const texts = open.user.data.content.filter((b) => b.type === "text").map((b) => b.text);
    if (texts.length > 0) {
      result.push({
        key: `${open.user.seq}:all`,
        turn: open.turn,
        eventSeq: open.user.seq,
        id: open.user.data.id,
        blockIndex: -1,
        kind: "user",
        text: texts.join("\n\n"),
        time: open.user.time,
...
// Plans: each returns { boundary, queuedUsers, manualTurn? } or a copy-edit plan.
function editPlan(operation, turns, events) {
  const event = events.find((candidate) => candidate.seq === operation.eventSeq && (candidate.type === "user/message" || candidate.type === "assistant/message"));
  ...
  if (event.type === "user/message") {
    // Edit in place: whole-session copy with the text swapped. The AI is NOT
    // re-invited — regenerating is a separate, explicit action (the reroll
    // button). Editing a message only changes its text.
...
function deletePlan(operation, turns, events) {
  const event = events.find((candidate) => candidate.seq === operation.eventSeq && (candidate.type === "user/message" || candidate.type === "assistant/message"));
  if (event === undefined) throw new Error("message not found");
  const turnIndex = turns.findIndex((t) => operation.eventSeq > t.startSeq && operation.eventSeq < t.endSeq);
  if (turnIndex === -1 && event.seq > (events.find((e) => e.type === "turn/start")?.seq ?? -1)) {
    throw new Error("message belongs to an unfinished turn");
  }
  if (event.type === "user/message") {
    // Delete ONLY this message: clone the whole closed session, drop the
    // event, renumber contiguously. Nothing is regenerated and nothing
    // downstream is removed.
    return { mode: "copy-edit", targetSeq: event.seq, remove: true };
  }
  // Delete an AI message = remove the WHOLE assistant output of its turn:
  // every assistant message (including the EMPTY tool-call skeleton rows that
  // otherwise stay visible on screen), steps, and tool records. The turn
  // keeps only its user message.
  const turn = turns[turnIndex];
  const removeSeqs = [];
  for (const candidate of events) {
    if (candidate.seq <= turn.startSeq || candidate.seq >= turn.endSeq) continue;
    if (candidate.type === "user/message" || candidate.type === "turn/end" || candidate.type === "agent/inbox/spliced") continue;
    removeSeqs.push(candidate.seq);
  }
  if (removeSeqs.length === 0) removeSeqs.push(event.seq);
  return { mode: "copy-edit", targetSeq: event.seq, removeSeqs, remove: true };
}

function rerollPlan(operation, turns, events) {
  // Regenerate from the EXACT user message whose reroll button was clicked
  // (operation.eventSeq), not always the last turn.
  if (operation && Number.isInteger(operation.eventSeq)) {
    const index = turns.findIndex((t) => operation.eventSeq > t.startSeq && operation.eventSeq < t.endSeq);
    const turn = turns[index];
    if (turn !== undefined && turn.user !== undefined && turn.user.seq === operation.eventSeq) {
      return { boundary: turn.startSeq - 1, queuedUsers: [cloneUser(turn.user.data)] };
    }
    // A stopped reply leaves the turn open: reroll from its user message by
    // cutting just before the open turn.
    const open = openTurnUser(events, turns);
    if (open !== undefined && open.user.seq === operation.eventSeq) {
      return { boundary: open.startSeq - 1, queuedUsers: [cloneUser(open.user.data)] };
    }
  }
...
function planOperation(operation, events) {
  ...
  if (operation.action === "edit") return editPlan(operation, turns, events);
  if (operation.action === "delete") return deletePlan(operation, turns, events);
  if (operation.action === "reroll") return rerollPlan(operation, turns, events);
...
// Renumber a derived event list contiguously from 0 and remap every seq
// reference (sourceEventSeqs, surfaceOp.replace start/end, messageSeqs,
// shadowedSeqs/Range) so the seed stays valid after events were removed.
function renumberEvents(events) {
  const seqMap = new Map();
  const result = [];
  for (const event of events) {
    seqMap.set(event.seq, result.length);
    result.push({ ...event, seq: result.length });
  }
  const mapSeq = (value) => seqMap.get(value);
  for (const event of result) {
    if (Array.isArray(event.sourceEventSeqs)) {
      event.sourceEventSeqs = event.sourceEventSeqs.map(mapSeq).filter((seq) => seq !== undefined);
    }
    if (event.surfaceOp && typeof event.surfaceOp === "object" && event.surfaceOp.op === "replace") {
      const start = mapSeq(event.surfaceOp.start);
      const end = mapSeq(event.surfaceOp.end);
      if (start === undefined || end === undefined || start > end) {
        // The replaced node is gone (should not happen: deletes only remove
        // current surface nodes, never replaced ones). Degrade to an append
        // so the replay validator still passes.
    ...
    if (event.data && typeof event.data === "object") {
      ...
      if (Array.isArray(data.messageSeqs)) data.messageSeqs = data.messageSeqs.map(mapSeq).filter((seq) => seq !== undefined);
      if (Array.isArray(data.shadowedSeqs)) data.shadowedSeqs = data.shadowedSeqs.map(mapSeq).filter((seq) => seq !== undefined);
      if (data.shadowedRange && typeof data.shadowedRange === "object") {
        const start = mapSeq(data.shadowedRange.start);
        const end = mapSeq(data.shadowedRange.end);
        if (start === undefined || end === undefined || start > end) delete data.shadowedRange;
        else data.shadowedRange = { start, end };
      }
      event.data = data;
    }
  }
...
function versionSeed(source, plan) {
  if (plan.mode === "copy-edit") {
    // Whole-session copy with one assistant message edited in place: swap the
    // target message's content, keep seqs. Delete mode drops the target event
    // instead and renumbers contiguously.
    // Copy up to the last closed turn, but never short of the target: a
    // message in a still-open turn (AI generating) is editable/deletable too —
    // the persistence layer closes interrupted turns on load.
    const end = Math.max(lastClosedEnd(source.events), plan.targetSeq);
    if (end < 0) throw new Error("session has no closed turn to copy");
    const prefix = source.events.slice(0, end + 1);
    if (plan.remove === true) {
      const remove = new Set(Array.isArray(plan.removeSeqs) && plan.removeSeqs.length > 0 ? plan.removeSeqs : [plan.targetSeq]);
      return {
        events: renumberEvents(prefix.filter((event) => !remove.has(event.seq))),
        inheritedLength: prefix.length - remove.size,
      };
    }
    const events = prefix.map((event) => {
      ...
      if (event.type === "user/message") {
        return Object.freeze({
          ...
          data: Object.freeze({
            ...event.data,
            content: Object.freeze(replaceMessageText(event.data.content, plan.text)),
          }),
        });
      }
      return Object.freeze({
...
export function apply(ctx) {
  ...
  // ---------- provider balance API (slow endpoints) ----------
  ...
  // Cache is long (5 min) with stale-while-revalidate: return cached data
  // immediately while refreshing async, so the UI never waits for slow APIs.
  const BALANCE_CACHE_TTL = 300000; // 5 min
  const BALANCE_TIMEOUT = 15000;    // 15 s (DeepSeek balance API is often slow)

  // Fallback key sources: try these config files when credentials.resolve fails.
  // This lets users whose keys live in plugin config (e.g. deepseek-router.json)
  // still see balances without duplicating env vars.
  const BALANCE_KEY_FALLBACKS = {
    OPENROUTER_API_KEY: {
      path: join(os.homedir(), ".dsh", "deepseek-router.json"),
      read: (cfg) => cfg.openrouterApiKey || "",
    },
    DEEPSEEK_API_KEY: {
      path: join(os.homedir(), ".dsh", "deepseek-router.json"),
      read: (cfg) => cfg.deepseekApiKey || "",
    },
  };

  /// Resolve an API key: credentials service first, then config-file fallbacks.
  async function resolveBalanceKey(keyRef) {
    try {
      const resolved = credentials ? await credentials.resolve(keyRef) : undefined;
      if (resolved && resolved.value) return resolved.value;
    } catch { /* fall through */ }
    const fallback = BALANCE_KEY_FALLBACKS[keyRef];
    if (fallback) {
      try {
        const raw = JSON.parse(readFileSync(fallback.path, "utf8"));
        const v = fallback.read(raw);
  ...
  const SPECS = [
    ...
    },
    {
      ...
      // Primary: /api/v1/credits returns {data: {total_credits, total_usage}}
      // showing free-tier credits (e.g. $10 total, $0.23 used → $9.77 remaining).
      ...
      parse: (j) => {
        ...
        const bal = total - (isFinite(used) ? used : 0);
        return { balance: Math.max(0, bal), currency: "USD", total: total, used: isFinite(used) ? used : null };
      },
      // Alt: /api/v1/auth/key returns usage + optional limit for key-based budgets.
      urlAlt: "https://openrouter.ai/api/v1/auth/key",
      parseAlt: (j) => {
        const d = j && j.data;
        if (!d) return null;
        const usage = Number(d.usage);
        if (!isFinite(usage)) return null;
        const limit = d.limit !== null && d.limit !== undefined ? Number(d.limit) : null;
        if (limit !== null && isFinite(limit)) {
          return { balance: Math.max(0, limit - usage), currency: "USD", total: limit, used: usage };
        }
        return null; // No limit = can't compute balance
      },
    },
    {
      id: "openai",
  ...
  let cache = null;
  let cacheAt = 0;
  let refreshing = false; // guard against concurrent background refreshes

  /// Fetch all provider balances concurrently.
  async function fetchAllBalances() {
    const out = {};
    const tasks = SPECS.map(async (spec) => {
      try {
        const apiKey = await resolveBalanceKey(spec.keyRef);
        if (!apiKey) return;
        ...
        const payloads = [];
        for (const u of urls) {
          const r = await fetch(u, { headers, signal: AbortSignal.timeout(BALANCE_TIMEOUT) });
        ...
        // Primary failed — try alt endpoint if present (e.g. /api/v1/credits for OpenRouter)
        if (spec.urlAlt && spec.parseAlt) {
          const r = await fetch(spec.urlAlt, { headers, signal: AbortSignal.timeout(BALANCE_TIMEOUT) });
          if (r.ok) {
            const parsed = spec.parseAlt(await r.json());
            if (parsed && parsed.balance !== null) {
              out[spec.id] = parsed;
              return;
            }
  ...
  const handler = async (req, res) => {
    ...
    try {
      const now = Date.now();
      // Stale-while-revalidate: return cache immediately if it exists, even if stale.
      if (cache !== null) {
        const stale = now - cacheAt;
        if (stale < BALANCE_CACHE_TTL) {
          // Fresh cache — return and done.
      ...
      // No cache at all — fetch synchronously (first load).
      const out = await fetchAllBalances();
      cache = out;
      cacheAt = now;
  ...
  const searchModelsHandler = async (req, res) => {
    ...
    if (req.method === "POST") {
      ...
      const sanitize = (m) => (m && typeof m === "object" ? {
        ...
        label: typeof m.label === "string" && m.label.trim() ? m.label.trim() : (m.type === "exa" ? "Exa" : m.type === "brave" ? "Brave" : m.type === "bing" ? "Bing" : m.type === "tavily" ? "Tavily" : m.type === "firecrawl" ? "Firecrawl" : m.type === "you" ? "You.com" : m.type === "serper" ? "Serper" : m.type === "serpapi" ? "SerpApi" : m.type === "kagi" ? "Kagi" : m.type === "searxng" ? "SearXNG" : m.type === "bocha" ? "Bocha" : m.model || "Search model"),
        type: m.type === "exa" ? "exa" : m.type === "brave" ? "brave" : m.type === "bing" ? "bing" : m.type === "tavily" ? "tavily" : m.type === "firecrawl" ? "firecrawl" : m.type === "you" ? "you" : m.type === "serper" ? "serper" : m.type === "serpapi" ? "serpapi" : m.type === "kagi" ? "kagi" : m.type === "searxng" ? "searxng" : m.type === "bocha" ? "bocha" : "deepseek",
        apiKey: typeof m.apiKey === "string" && m.apiKey.trim() ? m.apiKey.trim() : void 0,
        apiKeyEnv: typeof m.apiKeyEnv === "string" && m.apiKeyEnv.trim() ? m.apiKeyEnv.trim() : ({ you: "YOU_API_KEY", serper: "SERPER_API_KEY", serpapi: "SERPAPI_API_KEY", kagi: "KAGI_API_KEY", bocha: "BOCHA_API_KEY", searxng: "" }[m.type] || "DEEPSEEK_API_KEY"),
  ...
  async function runMessageEditOperation(operation) {
    ...
    return withSourceAgent(sessionId, async (source) => {
      ...
      try {
        ...
        if (plan.mode === "copy-edit") {
          // No followup runs for edits/deletes: the child agent stays idle but
          // ALIVE so the forked session remains in the live registry — the UI
          // list shows it and the client can switch to it. (Disposing it here
          // hid the new session from the list even though it was persisted.)
        }
  ...
  async function attachChildToWorkspace(sourceCwd, childId) {
    ...
    if (workspace === undefined || typeof workspace.attachSession !== "function") {
      console.error(`[ui-mobile] attachChildToWorkspace: no workspace matches ${canonical} (found ${list.length} workspaces)`);
      return;
    }
    try { await workspace.attachSession(childId); }
    catch (error) { console.error(`[ui-mobile] attachChildToWorkspace: attachSession failed for ${childId}: ${error instanceof Error ? error.message : String(error)}`); }
  }
  ...
  const messageEditHandler = async (req, res) => {
    ...
    try {
      ...
      if (req.method === "GET" || req.method === "HEAD") {
        ...
        res.writeHead(200);
        res.end(JSON.stringify({ messages: editableMessages(closedTurns(events), events) }));
        return;
      }
      if (req.method === "POST") {
        const chunks = [];

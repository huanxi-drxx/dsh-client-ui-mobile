...
export function apply(ctx) {
  ...
  // GET /api/dshm/tools-config — current disabled tool names.
  const toolsConfigGet = async (req, res) => {
    res.setHeader("content-type", "application/json");
    res.writeHead(200);
    res.end(JSON.stringify({ disabled: toolsDisabled }));
  };
  // POST /api/dshm/tools-config — set disabled tools; applies to a live session.
  const toolsConfigPost = async (req, res) => {
    res.setHeader("content-type", "application/json");
    try {
      const chunks = [];
  ...
  const disposeToolsConfigGet = webServer.register({ kind: "exact", path: "/api/dshm/tools-config", handler: toolsConfigGet });
  const disposeToolsConfigPost = webServer.register({ kind: "exact", path: "/api/dshm/tools-config", handler: toolsConfigPost });

  return () => {
    disposeBalances();
    disposeUpload();
    disposeTools();
    disposeToolsConfigGet();
    disposeToolsConfigPost();

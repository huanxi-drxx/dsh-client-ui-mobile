  ...
  async function runMessageEditOperation(operation) {
    ...
    return withSourceAgent(sessionId, async (source) => {
      ...
      try {
        ...
        if (plan.mode === "copy-edit") {
          // Editing an AI message spawns no generation: drop the child agent
          // right away so repeated edits do not accumulate live agents
          // (the session stays persisted; opening it resumes normally).

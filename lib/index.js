...
function loadSearchConfig() {
  try {
    ...
    const models = Array.isArray(j && j.models)
      ? j.models.filter((m) => m && typeof m === "object" && typeof m.model === "string" && m.model.length > 0)
      : [];
    if (models.length === 0) return { models: [DEFAULT_SEARCH_MODEL], current: DEFAULT_SEARCH_MODEL.id };
    const current = typeof j.current === "string" && models.some((m) => m.id === j.current) ? j.current : models[0].id;
    return { models, current };
  } catch { return { models: [DEFAULT_SEARCH_MODEL], current: DEFAULT_SEARCH_MODEL.id }; }
}
...
function createSearchProvider(credentialsSvc) {
  return {
    id: "dshm-search",
    available() {
      const m = currentSearchModel();
      return m !== null && typeof URL !== "undefined" && URL.canParse(m.baseURL || DEFAULT_SEARCH_MODEL.baseURL);
    },
    async search(request, signal) {
      const model = currentSearchModel();
      if (!model) throw new Error("no search model configured");
      const baseURL = model.baseURL || DEFAULT_SEARCH_MODEL.baseURL;
      ...
      if (credentialsSvc !== void 0) {
        try {
          ...
          apiKey = resolved && typeof resolved.value === "string" ? resolved.value : "";
        } catch { apiKey = ""; }
      }
      if (!apiKey) throw new Error(`search has no API key for "${apiKeyEnv}"`);
      const endpoint = baseURL.replace(/\/$/, "") + "/messages";
      const body = {
        model: model.model,
        max_tokens: model.maxTokens || DEFAULT_SEARCH_MODEL.maxTokens,
        messages: [{ role: "user", content: [{ type: "text", text: `Perform a web search for the query: ${request.query}` }] }],
        tools: [{ type: "web_search_20250305", name: "web_search", max_uses: model.maxUses || DEFAULT_SEARCH_MODEL.maxUses }],
      };
      let response;
      try {
        response = await fetch(endpoint, {
          ...
          headers: {
            ...
            "anthropic-version": model.apiVersion || DEFAULT_SEARCH_MODEL.apiVersion,
      ...
      return mapAnthropicSearchResponse(await response.json());
    },
  };
}
  ...
  const searchModelsHandler = async (req, res) => {
    ...
    if (req.method === "POST") {
      ...
      const sanitize = (m) => (m && typeof m === "object" ? {
        ...
        label: typeof m.label === "string" && m.label.trim() ? m.label.trim() : m.model || "Search model",
      ...
      if (Array.isArray(body.models)) {
        const models = body.models.map(sanitize).filter((m) => m !== null && m.model.length > 0);
        if (models.length === 0) {
          res.writeHead(400);

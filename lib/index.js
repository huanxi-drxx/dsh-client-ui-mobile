...
function loadSearchConfig() {
  try {
    ...
    const models = Array.isArray(j && j.models)
      ? j.models.filter((m) => m && typeof m === "object" && ((typeof m.model === "string" && m.model.length > 0) || m.type === "exa"))
      : [];
...
function createSearchProvider(credentialsSvc) {
  return {
    ...
    async search(request, signal) {
      ...
      if ((model.type || "deepseek") === "exa") return exaSearch(model, apiKey, request, signal);
  ...
  const searchModelsHandler = async (req, res) => {
    ...
    if (req.method === "POST") {
      ...
      const sanitize = (m) => (m && typeof m === "object" ? {
        ...
        label: typeof m.label === "string" && m.label.trim() ? m.label.trim() : (m.type === "exa" ? "Exa" : m.model || "Search model"),
        type: m.type === "exa" ? "exa" : "deepseek",
      ...
      if (Array.isArray(body.models)) {
        ...
        const models = body.models.map(sanitize).filter((m) => m !== null && (m.model.length > 0 || m.type === "exa"));
        if (models.length === 0) {
          res.writeHead(400);

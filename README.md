...
| ✏️ 消息编辑 | 每条 AI / 用户消息下方新增 **编辑 / 删除 / 重新生成** 按钮：**编辑** = 原地修改文本（不触发 AI 重新生成）；**删除** = 仅删除该消息（删除 AI 消息会连同该回合的流式块与工具调用记录一并移除）；**重新生成** = 从**被点击的那条用户消息**开始重新生成。所有操作都会创建新会话版本，源会话自动归档 |
...
- 消息下方的按钮：**✏️ 编辑**（修改文本，不重新生成）· **🗑 删除**（AI 消息连同其回合的工具记录一并删除）· **🔄 重新生成**（仅用户消息：从该条开始重新生成 AI 回复）
